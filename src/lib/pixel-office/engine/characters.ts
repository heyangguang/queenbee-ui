...
export function updateCharacter(
  ...
  if (ch.isCat || ch.isDog || ch.isBee) {
    updateCat(ch, dt, walkableTiles, tileMap, blockedTiles)
    return
  }

  let baseSpeedMult = 1
  if (!(ch as any).isCat && ch.isActive) baseSpeedMult = 2.5
  ...
  switch (ch.state) {
    ...
    case CharacterState.IDLE: {
      ...
      ch.wanderTimer -= dt
      if (ch.wanderTimer <= 0) {
        // Check if we've wandered enough — reset count and keep wandering (idle agents don't go back to desk)
    ...
    case CharacterState.WALK: {
      // Walk animation
      // if (ch.frameTimer >= WALK_FRAME_DURATION_SEC) { // Original line, handled by new animation logic
      //   ch.frameTimer -= WALK_FRAME_DURATION_SEC // Original line, handled by new animation logic
      //   ch.frame = (ch.frame + 1) % 4 // Original line, handled by new animation logic
      // }
      ...
      if (ch.path.length === 0) {
        ...
        if (ch.isActive) {
          ...
          if (ch.interactionTarget &&
            ...
            // 设置当前活动类型（触发活动气泡）
            ch.currentActivity = ch.interactionTarget.furnitureType || null
          ...
          // Check if arrived at assigned seat — only sit down if active
          if (ch.seatId) {
            const seat = seats.get(ch.seatId)
            if (seat && ch.tileCol === seatGridCol(seat) && ch.tileRow === seatGridRow(seat)) {
              // Not working — just stand here briefly then wander
              ch.state = CharacterState.IDLE
              ch.dir = seat.facingDir
              ch.wanderTimer = randomRange(WANDER_PAUSE_MIN_SEC, WANDER_PAUSE_MAX_SEC)
              ch.wanderCount = 0
      ...
      ch.moveProgress += (WALK_SPEED_PX_PER_SEC / TILE_SIZE) * dt * baseSpeedMult // Apply baseSpeedMult here
  ...
  // 边界裁剪 — 角色不能走出地图
  clampCharacterToMap(ch, tileMap)
}
...
/** 将角色位置裁剪到地图边界内 */
function clampCharacterToMap(ch: Character, tileMap: TileTypeVal[][]): void {
  const rows = tileMap.length
  const cols = rows > 0 ? tileMap[0].length : 0
  if (cols === 0 || rows === 0) return

  // 裁剪 tile 坐标
  ch.tileCol = Math.max(0, Math.min(cols - 1, ch.tileCol))
  ch.tileRow = Math.max(0, Math.min(rows - 1, ch.tileRow))

  // 裁剪 pixel 坐标
  const maxX = (cols - 1) * TILE_SIZE + TILE_SIZE / 2
  const maxY = (rows - 1) * TILE_SIZE + TILE_SIZE / 2
  ch.x = Math.max(TILE_SIZE / 2, Math.min(maxX, ch.x))
  ch.y = Math.max(TILE_SIZE / 2, Math.min(maxY, ch.y))
}
