...
const CODE_SNIPPET_LIFETIME = 3.5 // 秒（更短更紧凑）
const CODE_SNIPPET_SPAWN_RATE = 2.0 // 每秒生成数（更快的打字节奏）
const PHOTO_COMMENT_LIFETIME = 15.0 // seconds — SSE 推流和闲逛气泡都长时间保留
...
/** 按活动类型分组的对话/气泡文字 */
const ACTIVITY_SPEECHES: Record<string, string[]> = {
  // 📚 看书/书架
  'bookshelf': ['📚 这本不错', '📖 翻翻看', '🤔 学习一下', '📕 好久没看了', '💡 有意思'],
  'ts_library_gray_full': ['📚 查查资料', '📖 这本经典', '🔍 找找参考', '💡 原来如此'],
  // 🚿 饮水
  'ts_water_cooler': ['💧 喝口水', '🥤 渴了', '💧 补充水分', '☕ 来一杯'],
  'cooler': ['💧 喝点水', '🥤 好渴啊', '💧 休息一下'],
  // 📝 白板
  'whiteboard': ['📝 画个架构图', '🤔 这个方案...', '💡 有想法了', '✍️ 记一下', '📊 理理思路'],
  // 🖼️ 看画/照片
  'photograph': PHOTO_COMMENTS,
  'ts_painting_large_1': ['🖼️ 好看', '🎨 不错的画', '😊 放松一下眼睛'],
  'ts_painting_large_2': ['🖼️ 挺有意境', '🎨 谁画的？', '😌 赏心悦目'],
  'ts_painting_small_1': ['🖼️ 有意思', '🎨 小幅精品'],
  // 🛋️ 沙发
  'sofa': ['🛋️ 歇会儿', '😌 好舒服', '💤 有点困...', '😊 休息一下', '🧘 放松放松'],
  // 🌿 植物
  'plant': ['🌿 长得真好', '🌱 该浇水了', '🍃 呼吸新鲜空气', '😊 绿色养眼'],
  'ts_plant_small': ['🌱 小可爱', '🌿 生机勃勃', '😊 看看植物'],
  // 🕐 时钟
  'ts_clock': ['🕐 几点了？', '⏰ 快下班了', '🕐 时间过得好快', '😅 才这个点？'],
  // 💡 灯
  'lamp': ['💡 想想...', '🤔 思考中', '💭 让我想想', '😶 发个呆'],
  // 📞 电话
  'phone': ['📞 喂？你好', '📱 嗯嗯好的', '📞 我知道了', '😊 下午见'],
  // ☕ 咖啡
  'coffee': ['☕ 来杯咖啡', '🍵 续命', '☕ 美式还是拿铁？', '😊 太香了'],
  // 🍔 冰箱
  'ts_fridge': ['🍔 看看有什么吃的', '🥤 拿瓶饮料', '🍕 饿了', '🍪 零食时间！'],
  // 💻 看别人电脑
  'pc': ['💻 在写什么？', '👀 学习一下', '🤔 这个 bug...', '😮 厉害啊'],
  // 🖥️ 检查服务器
  'pc_back': ['🖥️ 检查一下', '📊 CPU 正常', '🔧 看看日志', '✅ 运行稳定', '🌡️ 温度正常'],
  'server_rack': ['🖥️ 巡检中...', '📊 负载正常', '🔧 查看日志', '✅ 全部在线', '🌡️ 温度 23°C', '💚 状态 OK'],
  // 🎮 装饰/其他
  'ts_deco_3': ['🎮 玩一会儿', '🕹️ 摸鱼时间', '😎 胜了！'],
  // 🏓 运动设施
  'ping_pong': ['🏓 来一局！', '🏓 接球！', '🏓 好球！', '😆 我赢了', '🔥 再来一局'],
  'basketball': ['🏀 投篮！', '🏀 进了！', '🏀 手感不错', '💪 再来一个'],
  'fitness': ['🏋️ 锻炼一下', '💪 加油！', '🏃 出出汗', '😤 再来一组', '🧘 拉伸一下'],
  'vending': ['🥤 买瓶水', '🥤 来瓶可乐', '🍫 来个零食', '💰 扫码支付'],
  // 🪟 窗边
  'window_gaze': ['🪟 外面天气真好', '😌 透透气', '🌤️ 风景不错', '💭 发个呆'],
  // 🚶 走廊
  'corridor_walk': ['🚶 走走', '😊 散个步', '🤝 路过打个招呼', '💪 活动活动'],
}

/** 闲逛时随机出现的心情/生活状态气泡 */
const IDLE_MOODS: string[] = [
  // 😊 开心
  '😊 今天心情不错', '😄 哈哈哈', '🥳 好开心', '✨ 状态很好',
  // 😮‍💨 疲惫
  '😮‍💨 有点累了', '🥱 好困...', '😪 需要休息', '💤 想打个盹',
  // 🍔 饿/吃
  '🍔 肚子饿了', '🍕 想吃披萨', '🍜 午饭吃什么？', '🍩 想吃甜的',
  // 🤔 思考
  '🤔 想想接下来做什么', '💭 发个呆', '🧐 刚才那个问题...', '💡 突然有个想法',
  // 📱 日常
  '📱 看看消息', '🎧 听首歌', '☕ 想喝咖啡', '🧘 深呼吸',
  // 🌈 感受
  '🌈 今天天气真好', '🌙 快下班了', '🌤️ 阳光不错', '🍃 好安静',
  // 💬 社交
  '👋 嘿！', '🤝 有空聊聊', '😎 最近怎么样？', '📢 有新需求来了吗？',
  // 🎯 工作相关
  '🎯 等待任务中...', '📋 看看还有啥要做的', '🔧 整理一下桌面', '📊 回顾一下进度',
]

/** 闲逛气泡触发概率（每秒） — 设低一点避免刷屏 */
const IDLE_MOOD_CHANCE_PER_SEC = 0.12  // 平均 8 秒出现一次
...
export class OfficeState {
  ...
  /** 当前交流连线：[charIdA, charIdB] */
  commLinks: Array<[number, number]> = []
  private nextSubagentId = -1
  private static CAT_ID = -9999
  private static DOG_ID = -9998
  private static BEE_ID = -9997
  ...
  private getFirstIdleHumanoid(): Character | null {
    const list = Array.from(this.characters.values())
      .filter(ch => !ch.isCat && !ch.isDog && !ch.isBee && ch.state === CharacterState.IDLE && ch.matrixEffect !== 'despawn')
      .sort((a, b) => a.id - b.id)
    return list[0] || null
  }
   ...
   * - 有 idle 玩家 → 跟随并保持半格距离
   */
  private updateBeeChase(ch: Character, dt: number, idleTarget: Character | null): void {
    if (!idleTarget) {
  ...
  /** 显示“收到任务”对话气泡 — 用 photoComments 系统显示一条对话 */
  showTaskBubble(id: number, text: string): void {
    const ch = this.characters.get(id)
    if (ch) {
      ch.photoComments = [{ text, age: 0, x: 0 }]
    }
  }
  ...
  update(dt: number): void {
    ...
    for (const ch of this.characters.values()) {
      ...
      if (ch.id === OfficeState.BEE_ID) {
        this.updateBeeChase(ch, dt, firstIdleHumanoid)
        continue
      }
      ...
      // 活动气泡 — 角色在交互点做活动时显示对应对话
      if (ch.currentActivity && ch.state === CharacterState.IDLE && !ch.isCat && !ch.isDog && !ch.isBee) {
        const speeches = ACTIVITY_SPEECHES[ch.currentActivity] || ['😊 ...']
      ...
      } else if (!ch.isCat && !ch.isDog && !ch.isBee && !ch.isActive && !ch.isSubagent) {
        // 闲逛状态 — 随机出现心情气泡（但 SSE 推送的对话优先级更高）
        ch.currentActivity = null
        for (const pc of ch.photoComments) pc.age += dt
        ch.photoComments = ch.photoComments.filter(pc => pc.age < PHOTO_COMMENT_LIFETIME)
        // 如果当前有 SSE 推送的气泡（fromSSE 标记），不生成闲逛气泡
        const hasSSEBubble = ch.photoComments.some(pc => pc.fromSSE)
        if (!hasSSEBubble && ch.photoComments.length === 0 && Math.random() < dt * IDLE_MOOD_CHANCE_PER_SEC) {
          ch.photoComments.push({
            text: IDLE_MOODS[Math.floor(Math.random() * IDLE_MOODS.length)],
            age: 0,
            x: (Math.random() - 0.5) * 16,
          })
        }
      } else {
        // 工作中或特殊角色 — 不显示闲聊气泡（工作气泡由 agentBridge 推送）
        if (!ch.isActive) {
          ch.currentActivity = null
          ch.photoComments = []
        }
      ...
      if (ch.isActive && ch.state === CharacterState.TYPE && !ch.isCat && !ch.isDog && !ch.isBee) {
        ...
        if (ch.codeSnippets.length < 4 && Math.random() < dt * CODE_SNIPPET_SPAWN_RATE) {
  ...
  getCharacterAt(worldX: number, worldY: number): number | null {
    ...
    for (const ch of chars) {
      ...
      if (ch.isCat || ch.isDog || ch.isBee) continue
