...
import { getTeams, createProject, type TeamConfig } from "@/lib/api";
import { Card, CardContent } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Badge } from "@/components/ui/badge";
import { ArrowLeft, Check, Loader2, Users, FolderKanban, GitBranch, FileText, Info } from "lucide-react";
...
export default function NewProjectPage() {
    ...
    const [form, setForm] = useState({ id: "", name: "", description: "", repo_path: "", teams: [] as string[] });
    ...
    const handleSave = useCallback(async () => {
        ...
        try {
            await createProject({ id: form.id, name: form.name, description: form.description, repo_path: form.repo_path, teams: form.teams });
        ...
        <div className="p-6 max-w-4xl space-y-6 animate-fade-in">
            ...
            <div className="relative overflow-hidden bg-gradient-to-r from-emerald-500/5 via-emerald-500/10 to-transparent border px-6 py-5">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-xl font-bold tracking-tight">新建项目</h1>
                        <p className="text-xs text-muted-foreground mt-0.5">创建一个新项目，关联 Team 实现记忆共享</p>
                    </div>
                </div>
            ...
            {/* 基本信息 */}
            <FormSection icon={FolderKanban} title="基本信息" description="项目的标识和名称">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <FormField label="项目 ID" required help="唯一标识，如 queenbee、office-ui">
                        <Input value={form.id} onChange={e => setForm({ ...form, id: e.target.value })} placeholder="queenbee" />
                    </FormField>
                    <FormField label="项目名称" required help="在界面上展示的友好名称">
                        <Input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} placeholder="QueenBee 引擎" />
                    </FormField>
                </div>
            </FormSection>

            {/* 描述与仓库 */}
            <FormSection icon={FileText} title="项目详情" description="描述和代码仓库路径（可选）">
                <FormField label="项目描述" help="简要说明项目的目的和范围">
                    <Input value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} placeholder="多 Agent 协调引擎项目" />
                </FormField>
                <FormField label="代码仓库路径" help="项目的本地代码仓库目录">
                    <div className="flex items-center gap-2">
                        <GitBranch className="h-4 w-4 text-muted-foreground shrink-0" />
                        <Input value={form.repo_path} onChange={e => setForm({ ...form, repo_path: e.target.value })} placeholder="/Users/xxx/project" />
                    </div>
                </FormField>
            </FormSection>

            {/* 关联 Team */}
            <FormSection icon={Users} title="关联 Team" description="选择参与此项目的 Team，Team 下所有 Agent 将共享项目级记忆">
